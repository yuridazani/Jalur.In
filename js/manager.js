...
(() => {
        ...
        sidebarNav.addEventListener('click', (e) => {
            ...
            switch (view) {
                ...
                case 'reports':
                    ...
                    break;
                case 'history':
                    renderHistoryView(); // PERBAIKAN: Menggunakan nama fungsi yang benar
                    break;
                case 'finance':
                    renderFinanceView(); // PERBAIKAN: Menggunakan nama fungsi yang benar
                    break;
                case 'drivers':
    ...
    // === PERBAIKAN: FUNGSI RENDER YANG HILANG ===
    
    // --- RENDER RIWAYAT PENGIRIMAN (YANG SELESAI) ---
    function renderHistoryView() {
        // Buat view baru untuk history jika belum ada
        createHistoryView();
            ...
            tableBody.innerHTML += `
                <tr class="border-b border-slate-200 dark:border-jalur-muted/20 hover:bg-slate-50 dark:hover:bg-jalur-dark/50">
                    ...
                    <td class="p-3 space-x-2">
                        <button data-action="view-transaction" data-transaction-id="${transaction.id}" class="text-sky-500 hover:text-sky-700" title="Lihat Detail">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button data-action="edit-transaction" data-transaction-id="${transaction.id}" class="text-amber-500 hover:text-amber-700" title="Edit">
                            <i class="fas fa-pencil-alt"></i>
                        </button>
                        <button data-action="delete-transaction" data-transaction-id="${transaction.id}" class="text-red-500 hover:text-red-700" title="Hapus">
    ...
    function updateReportsTable() {
        ...
        reports.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp)).forEach(report => {
            const reportDate = new Date(report.timestamp).toLocaleString('id-ID');
            const statusColor = report.status === 'Selesai' ? 'bg-green-500/10 text-green-500' : 'bg-amber-500/10 text-amber-500';
            const statusText = report.status || 'Menunggu';

            tableBody.innerHTML += `
                <tr class="border-b border-slate-200 dark:border-jalur-muted/20 hover:bg-slate-50 dark:hover:bg-jalur-dark/50">
                    <td class="p-3 text-sm">${reportDate}</td>
                    <td class="p-3 font-semibold">${report.driverName}</td>
                    <td class="p-3">${report.category}</td>
                    <td class="p-3 text-sm">${report.description}</td>
                    <td class="p-3">
                        <button data-action="view-image" data-img-src="${report.photoBase64}" class="text-sky-500 hover:text-sky-700">
                    ...
                    <td class="p-3 space-x-2">
                        <button data-action="resolve-report" data-report-id="${report.id}" 
                                class="text-green-500 hover:text-green-700 transition-colors ${report.status === 'Selesai' ? 'opacity-50' : ''}" 
    ...
    function handleDriverFormSubmit(e) {
        ...
        // Refresh current view if it's drivers view
        if (window.managerDrivers && !document.getElementById('manager-drivers-view').classList.contains('view-hidden')) {
            window.managerDrivers.renderDriversView();
        }
    ...
    function setupTransactionModalEvents() {
        ...
        document.getElementById('transaction-form').addEventListener('submit', (e) => {
            ...
            reader.onload = (e) => {
                ...
                // Update current view
                if (!document.getElementById('manager-dashboard-view').classList.contains('view-hidden')) {
                    renderManagerDashboard();
                } else if (!document.getElementById('manager-finance-view').classList.contains('view-hidden')) {
                    renderFinanceView();
                }
